...
const commandData = new SlashCommandBuilder()
    ...
    .addSubcommandGroup(g => g.setName('config').setDescription('Configure canned replies (requires Manage Server)')
        ...
        .addSubcommand(s => s.setName('cooldown').setDescription('Seconds between keyword auto-replies per channel')
            .addIntegerOption(o => o.setName('seconds').setDescription('0 = no cooldown').setRequired(true).setMinValue(0).setMaxValue(3600)))
        ...
        .addSubcommand(s => s.setName('debug').setDescription('Get debug.txt to share for troubleshooting'))
...
const HELP_TEXT = [
    ...
    '`/fyrxassisted config cooldown <segundos>` — espera entre auto-respuestas por canal',
    '`/fyrxassisted config logs <true|false>` — logs en consola (debug.txt siempre se guarda)',
    '`/fyrxassisted config debug` — te envía debug.txt para compartirlo',
...
async function handleInteraction(interaction) {
    ...
    logger.log(`[FyrxAssisted/CMD] user=${interaction.user.tag} guild=${guildId} /fyrxassisted config ${sub}`);

    if (sub === 'cooldown') {
        const seconds = interaction.options.getInteger('seconds', true);
        config.setCooldown(guildId, seconds * 1000);
        await ephemeralReply(`Cooldown de auto-respuestas: ${seconds}s por canal.`);
        return true;
    }
    if (sub === 'logs') {
    ...
    if (sub === 'debug') {
        await interaction.reply({ content: 'debug.txt (contiene fragmentos de mensajes; compártelo solo con quien te ayuda).', files: [logger.DEBUG_FILE], flags: MessageFlags.Ephemeral });
        return true;
    }
